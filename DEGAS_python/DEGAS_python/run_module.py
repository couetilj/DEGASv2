...
def run_model(opt, pat_expr_mat, pat_lab_mat, sc_expr_mat, sc_loc_mat = None, sc_lab_mat = None, sc_groups = None):
    # define data loaders and model
    high_reso_loader, low_reso_loader = load_datasets("train", opt, pat_expr_mat, pat_lab_mat, sc_expr_mat, sc_loc_mat, sc_lab_mat, sc_groups)
    high_reso_eval_loader, low_reso_eval_loader = load_datasets("eval", opt, pat_expr_mat, pat_lab_mat, sc_expr_mat, sc_loc_mat, sc_lab_mat, sc_groups)
...
def bagging_all_results(opt, pat_expr_mat, pat_lab_mat, sc_expr_mat, sc_loc_mat = None, sc_lab_mat = None, sc_groups = None):
    """
    ...
    if opt["tot_folds"] == 1:
        for seed in range(opt["tot_seeds"]):
            ...
            if "random_feat" in opt.keys() and opt["random_feat"] and "random_perc" in opt.keys():
                ...
                save_results_folder = run_model(opt, pat_expr_mat[:, select_feats], pat_lab_mat, sc_expr_mat[:, select_feats], sc_loc_mat, sc_lab_mat, sc_groups)
            else:
                save_results_folder = run_model(opt, pat_expr_mat, pat_lab_mat, sc_expr_mat, sc_loc_mat, sc_lab_mat, sc_groups)
    else:
